...
def pose_ren_net(net_type, iter_idx, output_dir):
    ...
    if net_type == 'train':
        ...
        n.pose, n.prev_pose, n.center = L.Slice(n.label, slice_param=dict(slice_dim=1, slice_point=[point_num_*3, point_num_*6]),
...
def make_solver(iter_idx, output_dir):
    solver_content = 'net: \"{0}/model/train_icvl_pose_ren_s{1}.prototxt\"\n\
test_iter: 12\n\
test_interval: 1000\n\
base_lr: 0.001\n\
lr_policy: \"step\"\n\
...
weight_decay: 0.0005\n\
snapshot: 40000\n\
snapshot_prefix: \"{0}/snapshot/icvl_pose_ren_s{1}\"'.format(output_dir, iter_idx)
    return solver_content


def make_pose_ren_net(output_dir, iter_num=2, test_iter_num=3):
    for iter_idx in xrange(1,iter_num+1):
        with open('{}/model/train_icvl_pose_ren_s{}.prototxt'.format(output_dir, iter_idx), 'w') as f:
            f.write(pose_ren_net('train', iter_idx, output_dir))
        with open('{}/model/solver_icvl_pose_ren_s{}.prototxt'.format(output_dir, iter_idx), 'w') as f:
            f.write(make_solver(iter_idx, output_dir))
    for iter_idx in xrange(1,test_iter_num+1):
        with open('{}/model/test_icvl_pose_ren_s{}_trainset.prototxt'.format(output_dir, iter_idx), 'w') as f:
            f.write(pose_ren_net('test-train', iter_idx, output_dir))
        with open('{}/model/test_icvl_pose_ren_s{}_testset.prototxt'.format(output_dir, iter_idx), 'w') as f:
