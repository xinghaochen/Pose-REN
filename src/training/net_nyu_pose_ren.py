...
def pose_ren_net(net_type, iter_idx, output_dir):
    ...
    root_folder_ = config.nyu_data_dir
    # data layers
    if net_type == 'train':
        ...
        n.data, n.label = L.PoseData(name="data", include=dict(phase=0),
                                     transform_param=dict(is_trans=False, trans_dx=10, trans_dy=10,
                                                          is_rotate=True, rotate_deg=15, is_zoom=True, zoom_scale=0.1),
        ...
        pose_data_param_test = dict(image_source='{}/cache/test_image.txt'.format(output_dir),
                                    label_source='{}/cache/test_label_s{}.txt'.format(output_dir, iter_idx),
                                    root_folder=root_folder_,
        ...
        pose_data_param_test = dict(image_source='{}/cache/train_image.txt'.format(output_dir),
                                    label_source=label_source_,
                                    root_folder=root_folder_,
                                    batch_size=128, shuffle=False, new_height=96, new_width=96,
                                    point_num=point_num_ * 2, point_dim=3, output_center=True,
    ...
    for idx in xrange(point_num_):
        #if idx == 9 or idx == 11 or idx == 12:
        if idx == 1 or idx == 2 or idx == 4: # old
            continue
    ...
    # connect_structure_1 = [[0, 1, 13], [2, 3, 13], [4, 5, 13], [6, 7, 13], [8, 10, 13]]
    connect_structure_1 = [[0,5,3], [0,13,12], [0,11,10], [0,9,8], [0,7,6]] # old annos
...
def make_pose_ren_net(output_dir, iter_num=2, test_iter_num=3):
    for iter_idx in xrange(1,iter_num+1):
        ...
        with open('{}/model/solver_nyu_pose_ren_s{}.prototxt'.format(output_dir, iter_idx), 'w') as f:
            f.write(make_solver(iter_idx, output_dir))
    for iter_idx in xrange(1,test_iter_num+1):
        with open('{}/model/test_nyu_pose_ren_s{}_trainset.prototxt'.format(output_dir, iter_idx), 'w') as f:
            f.write(pose_ren_net('test-train', iter_idx, output_dir))
        with open('{}/model/test_nyu_pose_ren_s{}_testset.prototxt'.format(output_dir, iter_idx), 'w') as f:
            f.write(pose_ren_net('test-test', iter_idx, output_dir))
